...
def Gene_GO_Matrix(Gene_Annotations, gene_names, save_path, GO = "BP", network = "PPI"):
    ...
    go_terms = list(set([item for sublist in Gene_Annotations.values() for item in sublist])) 
...
def GO_Embeddings(Matrix_Gene_GO, gene_embeddings, save_path, GO = "BP", network = "PPI", matrix = "PPMI",
    ...
    # In this case by default we used 500 reps, fix_U and initialitation of SVD.
        ...
        GO_embeddings = NMF_Implementation.NMF(Matrix_Gene_GO, len(gene_embeddings_db.columns), 
                               repetitions, init = init, fix_U = True, 
                               U = gene_embeddings)
...
def GO_Embeddings_Human(Matrix_Gene_GO, gene_embeddings, save_path, GO = "BP", network = "PPI", matrix = "PPMI",
    ...
    # In this case by default we used 500 reps, fix_U and initialitation of SVD.
        ...
        GO_embeddings = NMF_Implementation_Human.NMF(Matrix_Gene_GO, len(gene_embeddings_db.columns), 
                               repetitions, init = init, fix_U = True, 
                               U = gene_embeddings)
...
def Test_GO_Embeddings_Jaccard(data_path, dimensions, network = "PPI", matrix = "PPMI", enrichment = "BP",
                               intersection = False, soft_clustering = False):
    ...
    if soft_clustering == False:
        ...
        GO_embeddings = pd.read_csv(data_path + "_GO_Embeddings_" + str(enrichment) +
                                    "_" + str(network) + "_" + str(dimensions) + "_" + 
                                    str(matrix) + ".csv", index_col = 0, dtype={0: str})
        
    elif soft_clustering == True:
        
        GO_embeddings = np.load(data_path + "_Soft_Clustering_" + str(network) + "_" + str(matrix) + "_" + 
                                str(dimensions) + "_" + str(enrichment) + ".npy", allow_pickle = True)

    # Load dictionary:
    
    GO_enrichments = json.load(open(data_path + "Enrichments_"+ str(enrichment) + "_"
                               + str(network) + "_" + str(matrix) + "_" + str(dimensions) 
                               + "Per_Cluster.json"))
    ...
    if intersection == False:
        ...
        GO_enrichments_filt = set([item for sublist in GO_enrichments_filt for item in sublist])
...
def Mean_GO_Clusters(data_path, dimensions, network = "PPI", matrix = "PPMI", enrichment = "BP",
                               intersection = False):    
...
def Get_AUC_ROC_Curve(data_path, dimensions, network = "PPI", matrix = "PPMI", enrichment = "BP", p_value = False):
    
    # load the embeddings:
    
    GO_embeddings = pd.read_csv(data_path + "_GO_Embeddings_" + str(enrichment) +
                                "_" + str(network) + "_" + str(dimensions) + "_" + 
                                str(matrix) + ".csv", index_col = 0)
    ...
    Matrix_Gene_GO = pd.concat([pd.Series(v, name=k).astype(str) for k, v in GO_enrichments.items()], 
               axis=1)
    ...
    roc_scores = [roc_auc_score(Matrix_Gene_GO.T.loc[:,column],GO_embeddings.loc[:,int(column)]) for column in Matrix_Gene_GO.T.columns]
    np.mean(roc_scores)
...
def Get_AUC_PreRec_Curve(data_path, dimensions, network = "PPI", matrix = "PPMI", enrichment = "BP", random = True):
    ...
    for column in Matrix_Gene_GO.T.columns:
        precision, recall, thresholds = metrics.precision_recall_curve(Matrix_Gene_GO.T.loc[:,column],GO_embeddings.loc[:,int(column)], pos_label=1)
    ...
    if random == True:
        ...
        for rep in range(100): # 100 repetitions as a first step
            ...
            for column in Matrix_Gene_GO.T.columns:
                precision, recall, thresholds = metrics.precision_recall_curve(df.loc[:,column],GO_embeddings.loc[:,int(column)], pos_label=1)
                auc_score = metrics.auc(recall, precision)
        ...
        print(f"Auc is:", str(auc_list_mean), " and the auc of the random (10 rep is) ", str(auc_list_random_final_mean))
...
def Get_Soft_Clustering(data_path, dimensions, network = "PPI", matrix = "PPMI", enrichment = "BP"):
    ...
    np.save(data_path + "_Soft_Clustering_"+ str(network)+ "_" + str(matrix) + "_"  + str(dimensions) + "_" + str(enrichment) ,Clusters)
