...
os.chdir("./Cleaned_version/") # Choose the folder where the Scripts are:
...
save_path = "./Cleaned_version/Data/"
...
for control, tissue, cancer in zip(Control_list, Normal_tissue_list, Cancer_list):
    ...
    Cancer_Scripts_Human_Case_Control.Create_Tissue_Specific_Network(cancer, tissue, control, save_path, 
                                                                     plot = False)    
       
# 1.2 Plot the Venn Diagrams:

Cancer_Plot_Scripts.Venn_Diagram_Cancer_Networks(Cancer_list, Normal_tissue_list, Control_list)
...
Cancer_Plot_Scripts.Generate_PPMI(Cancer_list, Normal_tissue_list, Control_list, context_window = 10)

# 3. Generate the Gene embeddings:

dimension_list = [50, 100, 150, 200, 250, 300]

Cancer_Plot_Scripts.Generate_Gene_Embeddings(Cancer_list, Normal_tissue_list, 
                                             Control_list, dimension_list, max_iter=1000, verbose = 50)

# 4 Map Functional annotations (GO terms) to the embedding spaces:

Cancer_Plot_Scripts.Annotate_Gene_Space(Cancer_list, Normal_tissue_list, Control_list, 
                                        dimension_list, Annotation = "Leaf")
# 5 Generate the FMMs:

Cancer_Plot_Scripts.Embedding_Structure(Cancer_list, Normal_tissue_list, Control_list,
                                        dimension_list, Gene_Matrix = False, matrix = "PPMI", annotation = "Leaf")

# 6. Filter FMMs to contain only annotations shared by cancer and control.

Cancer_Plot_Scripts.Common_GO_Terms(Cancer_list, Normal_tissue_list, Control_list, annotation = "Leaf")
...
Cancer_Plot_Scripts.Parallel_Functional_Organization(Cancer_list, Normal_tissue_list, Control_list, dimension_list,
                    matrix = "PPMI", filtered = True, Common = True, Jaccard = True, number_similar = 500,annotation = "Leaf")
...
Cancer_Plot_Scripts.Parallel_Error(Cancer_list, Normal_tissue_list, Control_list, dimension_list,
                   comparison_list, common_list=None, matrix = "PPMI", filtered = True, Common = False, annotation = "Leaf")

# 8.2 Plot these errors:

Cancer_Plot_Scripts.Plot_Relative_Error(Cancer_list, Normal_tissue_list, Control_list, dimension_list,
...
Cancer_Plot_Scripts.Movement_Ranking(Cancer_list, Normal_tissue_list, 
...
Cancer_Plot_Scripts.Query_GO_terms(Normal_tissue_list) # Read the description of the funciton for details about the needed files.

# 9.5 Define cancer-related genes (based on the literature):

Cancer_Plot_Scripts.Query_Common_gene_set(Normal_tissue_list) # Read the description of the funciton for details about the needed files.

# 9.6 Assess the functional organization of genes and functions in the embedding space:

# Distance between genes and functions:

Cancer_Plot_Scripts.Gen_GO_Common_Space(Cancer_list, Normal_tissue_list, Control_list, optimal_dim)

# Demonstrate that the space is functionally organized (between genes and annotations):

Cancer_Plot_Scripts.Demonstrate_Gene_GO_Org(Cancer_list, Normal_tissue_list, Control_list, optimal_dim)
