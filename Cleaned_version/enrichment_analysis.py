...
def enrichment_analysis(clusters,annotation_mapping, dim, data_path, alpha=0.05, enrichment = "BP",  network = "PPI", matrix = "Adj", enriched='statistics',
                        get_p_values = True):
    ...
    save_path = data_path + "Enrichments_" + str(enrichment) + "_" + str(network) + "_" + str(matrix) + "_" + str(dim)
    
    # For the p_values:
    
    if get_p_values == True:
        p_values_df = pd.DataFrame(np.nan, index= set(annotation_list), columns = list(range(dim)))
    ...
    for number,cluster in enumerate(clusters):
        ...
        if N<5:
            ...
            if len(p_vals)==0:
                ...
                corrected_pvalues = multipletests(p_vals, alpha=alpha, method='fdr_bh', is_sorted=False, returnsorted=False)[1]
                enriched_terms = [terms_enriched[position] for position,i in enumerate(corrected_pvalues) if i<=alpha]
                ...
                if 'GO:0008150' in enriched_terms:
                    ...
                    print('Cluster %s enriched in the root of the ontology and %s more terms'%(number,len(enriched_terms)))

                enriched_term_in_each_cluster[number] = list(set(enriched_terms))

                for node in cluster:
                    try:
                        if len(set(filtered_mapping[node]).intersection(set(enriched_terms)))!=0:
    ...
    if enriched=='terms':
        ...
        enriched_go_terms = set([go_term for cluster in enriched_term_in_each_cluster.values() for go_term in cluster])
        ...
        annotated_clusters = [i for i in enriched_term_in_each_cluster.values() if len(i)!=0]
        percentage_of_enriched_clusters = round(len(annotated_clusters)/float(len(clusters))*100,2)
        percentage_of_enriched_genes =  round(len(enriched_genes_in_network)/float(len(filtered_mapping))*100,2)

        try:
            percentage_of_enriched_annotations = round(len(list(set(flatten(annotated_clusters))))/float(total_anotations_in_network)*100,2)
        except TypeError:
        ...
        table_Statistics = pd.DataFrame({"Enriched_Clusters" :  [percentage_of_enriched_clusters],
...
def embedding_space_enrichments(embedding_space,labels,annotation_mapping):
    ...
    percentage_of_enriched_clusters, percentage_of_enriched_annotations ,percentage_of_enriched_genes = enrichment_analysis(clusters_embedding_space,annotation_mapping, alpha=0.05, enriched='statistics')
    return percentage_of_enriched_clusters, percentage_of_enriched_annotations ,percentage_of_enriched_genes
