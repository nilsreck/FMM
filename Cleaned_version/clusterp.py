...
class PvClust:
    ...
    def _hc(self, n):
        ...
        temp = HierarchicalClusteringClusters(
            data.transpose(), self.method, self.metric
        )
    ...
    def _table(self):
        ...
        if self.parallel:
            ...
            with Pool() as pool:
                probabilities = pool.map(self._nbootstrap_probability, self.n_scaled)
    ...
    def plot(self, labels=None):
        ...
        plot_dendrogram(
            self.linkage_matrix, np.array(self.result[["AU", "BP"]]), labels
        )

    def seplot(self, save, tissue, pvalue="AU", annotate=False):
...
def plot_dendrogram(linkage_matrix, pvalues, labels=None):
    ...
    d = dendrogram(
        linkage_matrix, labels=labels, above_threshold_color="c", color_threshold=0.1
    )
    ax = plt.gca()
    for node, (x, y) in pos.items():
        if node == (len(pos.items()) - 1):
            ax.text(x - 6, y + 5, "AU", fontsize=11, fontweight="bold", color="purple")
            ax.text(x + 1, y + 5, "BP", fontsize=11, fontweight="bold", color="black")
        else:
